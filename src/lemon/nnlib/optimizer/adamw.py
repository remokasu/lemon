...
class AdamW(Optimizer):  # TODO : Create test cases
    """
    AdamW optimizer (Adam with decoupled Weight decay)

    AdamW fixes the weight decay implementation in Adam by decoupling it
    from the gradient-based update. This is the modern standard optimizer.
    ...
    to parameters, not to gradients:
        theta = theta - lr * (m_hat / (sqrt(v_hat) + eps) + weight_decay * theta)

    This is mathematically equivalent to L2 regularization.
    """
    ...
    def step(self):
        ...
        for i, param in enumerate(self.params):
            ...
            # Key difference from Adam: weight decay is applied to parameters directly
            param.data._data -= self.lr * (
                m_hat / (xp.sqrt(v_hat) + self.eps)
                + self.weight_decay * param.data._data
            )
